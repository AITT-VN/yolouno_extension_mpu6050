...
class HMC5883L:
    ...
    def __init__(self, address=30, gauss='1.3', declination=(0, 0)):
        ...
        self._i2c.writeto_mem(self._address, 0x00, pack('B', 0b111000))

        # Configuration register B:
        reg_value, self.gain = self.__gain__[gauss]
        self._i2c.writeto_mem(self._address, 0x01, pack('B', reg_value))

        # Set mode register to continuous mode.
        self._i2c.writeto_mem(self._address, 0x02, pack('B', 0x00))
        self._i2c.stop()
    ...
    def read(self):
        ...
        self._i2c.readfrom_mem_into(self._address, 0x03, data)
